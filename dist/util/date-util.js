...
function periodFromComponents(year, month, day, hour) {
    if (month != null) {
        month++;
    }
...
function getHourPeriod(dates) {
    return periodsFormat(dates, function (date) {
        return periodFromComponents(date.getFullYear(), date.getMonth(), date.getDate(), date.getHours());
    });
}
...
function getDate(period) {
    ...
    if (period === null || period === undefined || period.length < 4) {
        return null;
    }
